...
class ImplotConan(ConanFile):
  ...
  def generate(self):
    ...
    toolchain.variables["IMPLOT_SRC_DIR"] = self.source_folder.replace("\\", "/")
    if Version(self.version) < "0.16":
      ...
      toolchain.preprocessor_definitions["IMGUI_DEFINE_MATH_OPERATORS"] = ""
    toolchain.generate()
    deps = CMakeDeps(self)
    deps.generate()
  ...
  def _patch_sources(self):
    if Version(self.version) == "0.14" and Version(self.dependencies["imgui"].ref.version) >= "1.89.7":
      ...
      replace_in_file(self, os.path.join(self.source_folder, "implot.cpp"), "ImGuiButtonFlags_AllowItemOverlap", "ImGuiButtonFlags_AllowOverlap")
